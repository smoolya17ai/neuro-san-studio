...
class WebAgentNetworkBuilder:
    ...
    AGENT_INSTRUCTION_PREFACE = ("You represent the following content from a web page for the user and can answer "
    ...
    def _process_page(
        ...
        clean_text = text[:self.PAGE_LEN_MAX].replace('"', '').replace("'", "").encode("ascii",
        ...
        while to_visit and count < max_agents:
            ...
            try:
                ...
                if "text/html" not in resp.headers.get("Content-Type", ""):
                    continue
                count = self._process_page(url, parent_name, resp, visited, existing_names, agents, count, to_visit,
                                           base_domain)
    ...
    def main(cls):
        ...
        parser.add_argument("--total_agents", type=int, default=cls.TOTAL_AGENTS,
                            help=f"Total number of agents to generate (default: {cls.TOTAL_AGENTS})")
        parser.add_argument("--start_url", type=str, default=cls.START_URL,
                            help=f"Starting URL (default: {cls.START_URL})")
        parser.add_argument("--agent_network_name", type=str, default=cls.AGENT_NETWORK_NAME,
                            help=f"Agent network name (default: {cls.AGENT_NETWORK_NAME})")
        parser.add_argument("--max_children", type=int, default=cls.MAX_CHILDREN,
                            help=f"Maximum number of direct children per agent (default: {cls.MAX_CHILDREN})")
        parser.add_argument("--max_name_len", type=int, default=cls.MAX_NAME_LEN,
                            help=f"Maximum length for agent names (default: {cls.MAX_NAME_LEN})")
        parser.add_argument("--page_len_max", type=int, default=cls.PAGE_LEN_MAX,
                            help=f"Maximum length of text content for a page (default: {cls.PAGE_LEN_MAX})")
        parser.add_argument("--min_page_len", type=int, default=cls.MIN_PAGE_LEN,
                            help=f"Minimum required text length of a page (default: {cls.MIN_PAGE_LEN})")
        parser.add_argument("--politeness_delay", type=float, default=0.0,
                            help=
                            "Average delay (in seconds) between page requests to be polite to servers (default: 0.0)")
